...
void GUI::RenderInfo()
{
	...
	if (ImGui::Begin("Info", info_open, window_flags))
	{
		ImGui::TextColored(ImVec4(0.20f, 0.85f, 0.60f, 1.0f), "KestrelClient 0.0.1");
		ImGui::SameLine();
		ImGui::Text("|");
		ImGui::SameLine();
		ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.20f, 1.0f), "FPS: %d", Settings::DebugFPS);
		ImGui::SameLine();
		ImGui::Text("|");
		ImGui::SameLine();
		ImGui::TextColored(ImVec4(0.20f, 0.60f, 0.85f, 1.0f), "Ping: %d", Settings::DebugPing);
		ImGui::Separator();
	}
	//ImGuiStyle* style = &ImGui::GetStyle();
